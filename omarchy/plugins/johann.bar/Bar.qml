...
Item {
  ...
  component BarPanel: PanelWindow {
    ...
    implicitHeight: root.vertical ? 0 : root.barSize + 7
    color: "transparent"
    ...
    Loader {
      anchors.fill: parent
      anchors.topMargin: !root.vertical && root.position === "top" ? 5 : 0
      anchors.bottomMargin: !root.vertical && root.position === "top" ? 2 : 0
    ...
    Component {
      ...
      Item {
        ...
        Row {
          id: leftSections
          readonly property var entries: root.layoutEntries("left")
          readonly property int activeWindowIndex: root.entryIndex(entries, "omarchy.active-window")
          readonly property var activeWindowEntry: activeWindowIndex >= 0 ? entries[activeWindowIndex] : ({ id: "" })
          readonly property var beforeEntries: activeWindowIndex >= 0
            ? root.entriesBefore(entries, "omarchy.active-window")
            : entries
          readonly property var afterEntries: activeWindowIndex >= 0
            ? root.entriesAfter(entries, "omarchy.active-window")
            : []
          ...
          Item {
            visible: leftSections.beforeEntries.length > 0
            width: visible ? leftCoreModules.width + Style.space(8) : 0
            height: root.barSize
            ...
            ModuleList {
              id: leftCoreModules
              entries: leftSections.beforeEntries
              region: "left"
              anchors.centerIn: parent
            }
          }
          ...
          Item {
            visible: activeWindowSlot.width > 0
            width: visible ? activeWindowSlot.width : 0
            height: root.barSize
            ...
            ModuleSlot {
              id: activeWindowSlot
              entry: leftSections.activeWindowEntry
              region: "left"
              anchors.centerIn: parent
            }
          }
          ...
          Item {
            visible: leftSections.afterEntries.length > 0
            width: visible ? leftTrailingModules.width + Style.space(8) : 0
            height: root.barSize
            ...
            ModuleList {
              id: leftTrailingModules
              entries: leftSections.afterEntries
              region: "left"
              anchors.centerIn: parent
            }
          }
  ...
  component CenterModules: Item {
    ...
    Component {
      ...
      Item {
        ...
        readonly property real sectionWidth: centerRoot.hasAnchor
          ? centerBefore.width + centerAnchorModule.width + centerAfter.width
          : centerUnanchored.width

        Rectangle {
          x: centerRoot.hasAnchor
            ? centerAnchorModule.x - centerBefore.width - Style.space(6)
            : Math.round((parent.width - width) / 2)
          anchors.verticalCenter: parent.verticalCenter
          width: parent.sectionWidth + Style.space(12)
          height: root.barSize
          color: root.background
          radius: 8
          visible: width > 0
  ...
  component ModuleList: Loader {
    ...
    Component {
      ...
      Row {
        spacing: moduleListRoot.region === "right" ? Style.space(6) : 0
  ...
  component CustomCommandModule: WidgetButton {
    ...
    required property var entry
    property string moduleName: root.entryId(entry)
    property var settings: root.entrySettings(entry)
