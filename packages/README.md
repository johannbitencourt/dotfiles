...
The manifests are the source of truth for the package bootstrap. The install
script reads them directly, filters packages unavailable in configured pacman
repositories, and installs only the selected profiles.

## Required desktop

| Packages | Reason |
|---|---|
| `hyprland`, `uwsm` | Compositor and managed Wayland session |
...
| `wl-clipboard`, `cliphist`, `wtype` | DMS clipboard history, storage, and paste support |
...
- `development.txt`: Docker, Godot, GitHub and development TUI tools.
...
- `aur-base.txt`: AUR build prerequisites/helper, only requested when an AUR-backed profile or DMS greeter is selected.
